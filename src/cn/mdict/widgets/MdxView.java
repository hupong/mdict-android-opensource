...
public class MdxView extends RelativeLayout {
	...
	public interface MdxViewListener {
        //If events were handled by listener, the listener should return true. Otherwise should return true.
    ...
    public void switchViewMode(boolean listMode){
        ...
        if ( listMode ){
            ...
            if (entryViewList.getContainer()!=null && entryViewList.getContainer().getParent()!=null )
    ...
    public void displayByHeadword(String headword, boolean addToHistory){
    	if ( dict!=null && dict.isValid() ){
    		DictEntry entry=new DictEntry(DictEntry.kInvalidEntryNo, headword, dict.getDictPref().getDictId());
            if ( MdxDictBase.isMdxCmd(headword) ){
                entry.setEntryNo(DictEntry.kSystemCmdEntryNo);
            } else if ( dict.locateFirst(headword, true, false, entry)!=MdxDictBase.kMdxSuccess ){
    ...
    public void displayByEntry(DictEntry entry, boolean addToHistory ){
    	...
    	if (dict!=null && dict.isValid() ){
    		if ( entry.isValid() || entry.isSysCmd() ){ //TODO should handle syscmd here
                int dictId=dict.getDictPref().getDictId();
	        	if (!entry.isSysCmd() && (entry.getDictId()!=dictId || (!dict.canRandomAccess() && entry.getSiblingCount()==0))){
	        		displayByHeadword(entry.getHeadword(), false);
	        	}else{
                    if ( mdxViewListener !=null ){
                        if (mdxViewListener.onDisplayEntry(this, currentEntry, addToHistory))
                            return;
                    }
                    if ( MdxEngine.getSettings().getPrefHighSpeedMode() ){
                        ...
                        //htmlView.loadUrl(String.format("content://mdx.mdict.cn/%d/%d/%s/", currentEntry.getDictId(), currentEntry.getEntryNo(), currentEntry.getHeadword()));
                        ...
                        if (currentEntry.isUnionDictEntry()){
                            ...
                            switchViewMode(false);
                            entryViewSingle.loadUrl(String.format("content://mdx.mdict.cn/%d/%d/%s/", currentEntry.getDictId(), currentEntry.getEntryNo(), currentEntry.getHeadword()));
                        }
                    }
                }
    		} else {
                String str=String.format( getContext().getString(R.string.headword_not_found), currentEntry.getHeadword());
    			displayHtml(str);
        	}
    ...
    private void updateUIStateForEntry(DictEntry entry) {
        if ( btnAddToFav!=null && btnSpeak!=null ){
            if ( entry!=null){
                ...
                boolean showSpeak= (ttsEngine!=null) && MdxEngine.getSettings().getPrefUseTTS() ;
                showSpeak = showSpeak|| AddonFuncUnt.hasSpeechForWord(dict, entry.getHeadword());
...
/*
   ...
   */
    ...
    private void setupControls(){
        ...
        entryViewList=new EntryViewList(getContext(), (ScrollView)findViewById(R.id.entryListView));

        entryViewList.setMdxView(this);
        entryViewSingle=new EntryViewSingle(getContext(), (WebView)findViewById(R.id.entrySingleView));
    ...
    public void addCurrentEntryToFav() {
        if ( currentEntry.isValid() ){
            ...
            Toast.makeText(getContext(), 
                    String.format(getResources().getString(R.string.entry_added_to_fav), currentEntry.getHeadword()),
                    Toast.LENGTH_SHORT).show();
        }
    }

    public void playPronunciationForCurrentEntry(){
        if ( currentEntry!=null && currentEntry.isValid() && currentEntry.getHeadword().length()!=0 ){
            if ( !AddonFuncUnt.playAudioForWord(dict, currentEntry.getHeadword()) && ttsEngine!=null && MdxEngine.getSettings().getPrefUseTTS() ){
                String headword=currentEntry.getHeadword().trim();
                StringBuffer hw=new StringBuffer(currentEntry.getHeadword().length());
                char c;
                ...
                if ( hw.length()>0 )
                    ttsEngine.speak(hw.toString(), TextToSpeech.QUEUE_FLUSH, null);
                else
