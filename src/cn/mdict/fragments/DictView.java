...
public class DictView extends SherlockFragment implements MdxViewListener, SimpleActionModeCallbackAgent.ActionItemClickListener,
        TextToSpeech.OnInitListener, WebViewGestureFilter.GestureListener {
    @Override
    public boolean onSearchText(MdxView view, String text, int touchPointX, int touchPointY) {
        //TODO handle in dict lookup by popup window
        return false;
    }

    @Override
    public boolean onDisplayEntry(MdxView view, DictEntry entry, boolean addToHistory) {
        ...
        if (btnAddToFav != null)
            btnAddToFav.setEnabled(currentEntry != null && currentEntry.isValid());
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        customViewForDictView = (ViewGroup) inflater.inflate(R.layout.search_edit, null);
        ...
        //actionModeAgent=new SimpleActionModeCallbackAgent(R.menu.app_menu, this);
        ...
        btnAddToFav = (ImageButton) customViewForDictView.findViewById(R.id.add_to_fav);
        if (btnAddToFav != null) {
        ...
        inputBox = (EditText) (customViewForDictView.findViewById(R.id.search_field));
        ...
        adapter.setDict(dict, getString(R.string.empty_entry_list), getString(R.string.invalid_dict));
        ...
        headwordList.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position,
                ...
                InputMethodManager imm = (InputMethodManager) getSherlockActivity().getSystemService(android.content.Context.INPUT_METHOD_SERVICE);
        ...
        focusChangeListener = new TextView.OnFocusChangeListener() {
            ...
            public void onFocusChange(android.view.View view, boolean hasFocus) {
                ...
                btnClear.setVisibility(inputBox.getText().length() != 0 ? View.VISIBLE : View.INVISIBLE);
                getSherlockActivity().invalidateOptionsMenu();
                ...
                if (!skipUpdateEntryList) {
                    ...
                    if (dict != null && dict.isValid()) {
                        ...
                        if (!currentEntry.isValid())
                            adapter.setCurrentEntry(new DictEntry(0, "", dictPref != null ? dictPref.getDictId() : DictPref.kInvalidDictPrefId));
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        //rebuildOptionMenuOnDemand(menu, getSupportActivity().getMenuInflater(), false);

        boolean show_toolbar = MdxEngine.getSettings().getPrefShowToolbar();
        for (int i = 0; i < menu.size(); ++i) {
            menu.getItem(i).setShowAsAction(show_toolbar ? MenuItem.SHOW_AS_ACTION_IF_ROOM : MenuItem.SHOW_AS_ACTION_NEVER);
        }
        MenuItem item;
        ...
        enableMenuItem(menu, R.id.add_to_fav, currentEntry != null && currentEntry.isValid());
        ...
        if (item != null) {
            ...
            if (submenu != null) {
                ...
                if (menuItem != null)
                    menuItem.setEnabled(zoomLevel >= 0 && zoomLevel < DictPref.kZoomLargest);
...
//                return true;
    ...
    public boolean hasPronunciationForCurrentEntry() {
        boolean hasSound = dict != null && dict.isValid() && currentEntry != null && currentEntry.isValid() && currentEntry.getHeadword().length() > 0;
        if (hasSound)
            hasSound = (ttsEngine != null && MdxEngine.getSettings().getPrefUseTTS()) || AddonFuncUnt.hasSpeechForWord(dict, currentEntry.getHeadword());
        return hasSound;
    }
    ...
    public void selectDict(int dictId) {
        ...
        int result = MdxEngine.openDictById(dictId, MdxEngine.getSettings().getPrefsUseLRUForDictOrder(), dict);
        setDict(dict);
        if (result == MdxDictBase.kMdxSuccess) {
            ...
            AddonFuncUnt.showMessageDialog(getSherlockActivity(), info, getString(R.string.error));
        }
    }
    ...
    public void switchToContentView() {
        ...
        //InputMethodManager imm = (InputMethodManager) getSupportActivity().getSystemService(android.content.Context.INPUT_METHOD_SERVICE);
        //imm.hideSoftInputFromWindow(inputBox.getWindowToken(), 0);
    ...
    public void displayWelcome() {
        ...
        AddonFuncUnt.loadStringFromAsset(getSherlockActivity().getAssets(), "Welcome.htm", welcome, true);
        String html = welcome.toString().replace("$version$", AddonFuncUnt.getVersionName(getSherlockActivity()));
        displayHtml(html);
    }
    ...
    public void onSwipe(View view, int direction, int touchPointCount, MotionEvent motionEvent) {
        if (touchPointCount == 1) {
            ...
            if (dict != null && dict.isValid() && dict.canRandomAccess() && currentEntry.isValid()) {
                if (direction == WebViewGestureFilter.GestureListener.swipeLeft) {
                    contentView.displayEntryNext();
                } else {
                    contentView.displayEntryPrev();
    ...
    private void toggleToolbarVisible() {
        MdxEngine.getSettings().setPrefShowToolbar(!MdxEngine.getSettings().getPrefShowToolbar());
        getSherlockActivity().invalidateOptionsMenu();
    }
    ...
    public void onInit(int i) {
        if (i == TextToSpeech.ERROR) {
            ...
            if (MdxEngine.getSettings().getPrefPreferedTTSEngine().length() != 0)
                ttsEngine.setEngineByPackageName(MdxEngine.getSettings().getPrefPreferedTTSEngine());
